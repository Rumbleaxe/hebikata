...
Architecture:
    - Exercises are defined in individual YAML files (data/exercises/*.yaml)
...
def load_exercises() -> List[Dict[str, Any]]:
    ...
    Reads data/index.yaml for the ordered list of exercise refs, then loads
    each data/exercises/{ref}.yaml file. Each exercise contains nested keys
    under metadata, content, validation, and hints.
    ...
    for entry in index["exercises"]:
        ref = entry["ref"]
        ex_path = data_dir / "exercises" / f"{ref}.yaml"
...
def execute_code_with_tests(user_code: str, test_code: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Find all test functions in the namespace
        ...
        # Validate that we found at least one test function
        if not test_funcs:
            return {
                "success": False,
        ...
        # Run all test functions - will raise AssertionError on first failure
...
def main():
    ...
    st.markdown(
        '<div style="text-align: center; color: #00FF41; font-family: monospace; font-size: 18px; margin-bottom: 30px;">PYTHON KATA DOJO - Master through repetition</div>',
        unsafe_allow_html=True,
    )
    ...
    with col1:
        st.markdown(
            f'<div class="stats-text">❤️ Lives: {st.session_state.lives}</div>',
            unsafe_allow_html=True,
        )
    with col2:
        st.markdown(
            f'<div class="stats-text">🎯 Score: {st.session_state.score}</div>',
            unsafe_allow_html=True,
        )
    with col3:
        current_idx = st.session_state.current_exercise_idx
        st.markdown(
            f'<div class="stats-text">📚 Exercise: {current_idx + 1}/{len(st.session_state.exercises)}</div>',
            unsafe_allow_html=True,
        )
    with col4:
        successes = st.session_state.successes[current_idx]
        st.markdown(
            f'<div class="stats-text">✅ Progress: {successes}/3</div>',
            unsafe_allow_html=True,
        )
    ...
    with left_col:
        ...
        st.markdown(
            f'<div class="exercise-title">Exercise {current_idx + 1}: {current_exercise["metadata"]["theme"].upper()}</div>',
            unsafe_allow_html=True,
        )
        st.markdown(
            f'<div class="prompt-text">{current_exercise["content"]["prompt"]}</div>',
            unsafe_allow_html=True,
        )
        ...
        with btn_col3:
            if st.button("💡 Show Hint", use_container_width=True):
                st.info(f"**Hint:** {current_exercise['hints'][0]['text']}")
    ...
    with right_col:
        ...
        for idx, exercise in enumerate(st.session_state.exercises):
            ...
            st.markdown(
                f"{current_marker}{status} **{exercise['metadata']['theme'].title()}** - {success_count}/3 successes ({attempt_count} attempts)"
            )
        ...
        with nav_col1:
            ...
            if st.button(
                "⬅️ Previous", disabled=current_idx == 0, use_container_width=True
            ):
                previous_exercise()
                st.rerun()
        with nav_col2:
            ...
            if st.button(
                "Next ➡️",
                disabled=current_idx == len(st.session_state.exercises) - 1,
